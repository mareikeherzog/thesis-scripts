...
# Name: merge_bams_samtools.pl bams_for_mplieup
...
while ($i < $loop_length) {
	...
	# if they are we build commands
		...
		if  ($flag == 0) {
			$command = "samtools merge "."$paths[$k]"."$samples[$k]".'/'."$samples[$k]".'.bam '."$files[$k] $files[$i]";	
		}
