...
#Name: filter_bait_regions.pl <bed_file> <vcf>

#Description: Script to remove any variants from a vcf not within the bait region of a mouse exome    
...
#give location of the file that contains all the info about exon bait locations: 3073253 3074322
...
if( $conversion_file =~ /\.gz/ ){open($ifi, qq[gunzip -c $conversion_file|]);}else{open($ifi, $conversion_file ) or die $!;}
        ...
        while( my $line = <$ifi> ) {
                ...
                #$f1 should be chr, $f2 should be start, $f3 is stop
        		# make specfic arrays for each chromosome
        		#f1 is the chromosome, f4 the start and f5 the end of an exon
        		#print("F1 = $f1 \t F2 = $f2 \t F3 = $f3 \n");
        		if ($f1 eq '1') {push @chr1_s, $f2 ; push @chr1_e, $f3; $f1=0; $f2=0; $f3=0;}
        		elsif ($f1 eq '2') {push @chr2_s, $f2 ; push @chr2_e,$f3; $f1=0; $f2=0; $f3=0;}
        		elsif ($f1 eq 3) {push @chr3_s, $f2 ; push @chr3_e, $f3;}
        		elsif ($f1 eq 4) {push @chr4_s, $f2 ; push @chr4_e, $f3;}
        		elsif  ($f1 eq 5) {push @chr5_s, $f2 ; push @chr5_e, $f3;}
        		elsif  ($f1 eq 6) {push @chr6_s, $f2 ; push @chr6_e, $f3;}
        		elsif ($f1 eq 7) {push @chr7_s, $f2 ; push @chr7_e, $f3;}
        		elsif ($f1 eq 8) {push @chr8_s, $f2 ; push @chr8_e, $f3;}
        		elsif ($f1 eq 9) {push @chr9_s, $f2 ; push @chr9_e, $f3;}
        		elsif ($f1 eq 10) {push @chr10_s, $f2 ; push @chr10_e, $f3;}
        		elsif ($f1 eq 11) {push @chr11_s, $f2 ; push @chr11_e, $f3;}
        		elsif ($f1 eq 12) {push @chr12_s, $f2 ; push @chr12_e, $f3;}
        		elsif ($f1 eq 13) {push @chr13_s, $f2 ; push @chr13_e, $f3;}
        		elsif ($f1 eq 14) {push @chr14_s, $f2 ; push @chr14_e, $f3;}
        		elsif ($f1 eq 15) {push @chr15_s, $f2 ; push @chr15_e, $f3;}
        		elsif ($f1 eq 16) {push @chr16_s, $f2 ; push @chr16_e, $f3;}
        		elsif ($f1 eq 17) {push @chr17_s, $f2 ; push @chr17_e, $f3;}
        		elsif ($f1 eq 18) {push @chr18_s, $f2 ; push @chr18_e, $f3;}
        		elsif ($f1 eq 19) {push @chr19_s, $f2 ; push @chr19_e, $f3;}
        		elsif ($f1 eq 'X') {push @chrX_s, $f2 ; push @chrX_e, $f3;}
